...
2. **推理引擎支持**：
   ...
   vLLM 和 SGLang 引擎均自动读取 Profiler 配置，无需额外设置。具体差异如下：

   - **vLLM 引擎**
      - 自动采集 ``AsyncLLM`` 调度栈及推理进程性能数据。不支持设置 ``analysis``（默认不解析，需离线解析）和 ``profiler_level``（默认 level1）。

   - **SGLang 引擎**
      - 自动读取配置文件中的配置。
      - 注意：SGLang 当前不支持 ``contents`` 中的 ``memory`` 配置项。不支持设置 ``analysis``（默认解析）和 ``profiler_level``（默认 level0）。
